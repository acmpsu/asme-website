  ...
  return (
    ...
    <header
      ...
      style={{
        ...
        // If on home page, add a gradient overlay that stays dark until 75%, then fades to transparent.
        // Otherwise, make it opaque.
        background: isHomePage
          ? "linear-gradient(180deg, rgba(0, 0, 0, 0.5) 0%, rgba(0, 0, 0, 0.5) 75%, rgba(0, 0, 0, 0) 100%)"
          : "linear-gradient(180deg, #1E2B48 75.96%, var(--Light-YinMin, #899ABD) 100%)",
        // Add backdrop blur for better transparency effect (only on home page)
        backdropFilter: isHomePage ? "blur(2px)" : "none",
        WebkitBackdropFilter: isHomePage ? "blur(2px)" : "none",
        // Add a blue line at the bottom of the header (only when not on home page)
